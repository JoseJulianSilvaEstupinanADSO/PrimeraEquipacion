...
// Elementos para agregar y modificar productos
...
// Elementos para la tabla y la búsqueda
...
// Evento para abrir el modal de modificar producto al hacer clic en el botón de editar
...
// Listar las tallas en el select
...
async function AgregarModificarProducto() {
    ...
    if ($bnt_modal.classList.contains('agregar__producto')) {
        if (num === $inputs.length && $estado != "Seleccionar" && $talla_producto != "Seleccione la talla") {
            ...
            if (respuesta.resultado) {
                Mensaje($titleError, $paragrahp, error, "Éxito", "Producto agregado");
                CloseModal();
                ListarProductos();
            } else {
    ...
    if ($bnt_modal.classList.contains('modificar__producto')) {
        ...
        if (respuesta.resultado) {
            Mensaje($titleError, $paragrahp, error, "Éxito", "Producto modificado");
            CloseModal();
            ListarProductos();
        } else {
...
async function ListarProductos() {
    ...
    respuesta.forEach((x) => {
        ...
        let valor = x.estado === "1" ? "Habilitado" : "Deshabilitado";
...
// Buscar producto específico en la tabla
function Buscar() {
